-- Fix regression: 20260910130000_proposal_legal_annex recreated
-- update_proposal_items_versioned without payment_plan, so any item save from
-- the editor (which always sends the plan in p_patch) failed with
-- "Campos de propuesta no válidos". Restore the field in the patch allowlist
-- and in the proposals update, keeping the legal_terms support intact.
...
begin
  ...
  if jsonb_typeof(p_patch) <> 'object'
     or p_patch - array[
       ...
       'payment_plan', 'payment_terms', 'change_management_terms', 'maintenance_options',
