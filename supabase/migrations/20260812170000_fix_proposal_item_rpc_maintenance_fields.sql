-- The commercial-scope and maintenance migrations were originally assigned
-- the same version. Depending on the migration runner, the RPC could retain
-- the former allowlist and reject the maintenance payload sent by autosave.
-- Recreate the final definition in a uniquely-versioned migration so either
-- previous state is repaired safely.
