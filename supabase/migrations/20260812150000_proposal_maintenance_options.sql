-- Optional, proposal-scoped maintenance offer. The JSONB snapshot means a
-- client always sees the exact coverage and price offered for this proposal.
alter table public.proposals
  add column if not exists maintenance_options jsonb,
  add column if not exists maintenance_selected_plan_id text,
  add column if not exists maintenance_selection_source text,
  add column if not exists maintenance_selected_at timestamptz;

alter table public.proposals
  drop constraint if exists proposals_maintenance_selection_source_check;

alter table public.proposals
  add constraint proposals_maintenance_selection_source_check
  check (maintenance_selection_source is null or maintenance_selection_source in ('team', 'client'));

comment on column public.proposals.maintenance_options is
  'Proposal-scoped editable snapshot of the optional web maintenance plans.';
comment on column public.proposals.maintenance_selected_plan_id is
  'Optional selected maintenance plan id from maintenance_options.';

-- The existing RPC pre-dated commercial-scope fields and rejected them whenever
-- an autosave included line items. Keep the whitelist and write logic in sync.
...
begin
  ...
  subtotal := v_subtotal; tax_amount := v_tax_amount; total := v_total; return next;
end;
$$;