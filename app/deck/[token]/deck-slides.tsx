...
function SectionSlide({
  ...
  return (
    <div className={`deck-slide justify-center text-zinc-900 p-6 sm:p-10 md:p-16 lg:p-24 ${bg}`}>
      ...
      <Stagger i={2} className="w-full flex flex-col items-center">
        {children}
      </Stagger>
    </div>
  );
        ...
        <table className="w-full text-xs sm:text-sm md:text-base mb-6 sm:mb-8 text-left">
          <thead>
            <tr className="border-b border-zinc-200">
              <th className="text-left py-2 sm:py-3 text-zinc-500 font-medium uppercase tracking-wider text-[10px] sm:text-xs">
                Descripción
              </th>
              <th className="hidden sm:table-cell text-right py-3 text-zinc-500 font-medium uppercase tracking-wider text-xs">
                Cant.
              </th>
              <th className="hidden sm:table-cell text-right py-3 text-zinc-500 font-medium uppercase tracking-wider text-xs">
                Precio
              </th>
              <th className="text-right py-2 sm:py-3 text-zinc-500 font-medium uppercase tracking-wider text-[10px] sm:text-xs">
                Subtotal
              </th>
            </tr>
          </thead>
            ...
            {items.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="hidden sm:table-cell py-4 text-right tabular-nums text-zinc-500">
                    {item.quantity}
                  </td>
                  <td className="hidden sm:table-cell py-4 text-right tabular-nums text-zinc-500">
                    {formatEUR(item.unit_price)}
                  </td>
                  <td className="py-3 sm:py-4 text-right tabular-nums font-medium whitespace-nowrap">
        ...
        {team.map((member, i) => (
          ...
          >
            ...
            <div className="text-center">
              ...
              {member.job_title && (
                <p className="text-[11px] sm:text-xs text-zinc-500 mt-0.5">{member.job_title}</p>
              )}
            </div>
          </div>
        ))}
              ...
              <div key={item.id} className="deck-stagger" style={{ ["--i" as string]: 3 + i }}>
                <div className="flex justify-between gap-3 mb-1.5">
                  <div className="flex items-center gap-2 min-w-0">
                    <span className="text-xs sm:text-sm text-zinc-700 truncate text-left">
                      {item.description}
                    </span>
                    <CadenceBadge cycle={cycle} />
                  </div>
                  <span className="text-xs sm:text-sm font-semibold tabular-nums text-zinc-900 whitespace-nowrap">
                    {formatEUR(item.subtotal)}
                  </span>
                </div>
                <div className="h-1.5 sm:h-2 bg-zinc-100 rounded-full overflow-hidden">
                  <div
                    className="h-full bg-[#2A4227] rounded-full"
...
function ClosingSlide({ proposal, token }: { proposal: DeckProposal; token: string }) {
  return (
    <div className="deck-slide bg-[#2A4227] text-white p-6 sm:p-10 md:p-16 lg:p-24 text-center">
      ...
      {proposal.client_name && (
        <Stagger i={4}>
          <p className="mt-10 sm:mt-16 text-[10px] sm:text-xs text-white/40 uppercase tracking-[0.25em] sm:tracking-[0.3em]">
            {proposal.client_name}
          </p>
        </Stagger>
      )}
