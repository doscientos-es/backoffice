...
function IntroSlide({ proposal }: { proposal: DeckProposal }) {
  return (
    <SectionSlide label="Sobre el proyecto" title="El contexto">
      <div className="max-w-3xl text-left w-full">
        <Markdown
          source={proposal.intro ?? ""}
...
function PricingTotals({ proposal }: { proposal: DeckProposal }) {
  return (
    <div className="flex flex-col gap-2 items-end w-full">
      <div className="flex gap-6 sm:gap-12 text-xs sm:text-sm text-zinc-500">
        <span>Subtotal</span>
        <span className="tabular-nums">{formatEUR(proposal.subtotal)}</span>
      </div>
      <div className="flex gap-6 sm:gap-12 text-xs sm:text-sm text-zinc-500">
        <span>IVA</span>
        <span className="tabular-nums">{formatEUR(proposal.tax_amount)}</span>
      </div>
      <div className="flex gap-6 sm:gap-12 text-xl sm:text-2xl md:text-3xl font-bold text-zinc-900 border-t-2 border-zinc-300 pt-3 mt-2">
        <span>Total</span>
        <span className="tabular-nums">{formatEUR(proposal.total)}</span>
      </div>
    </div>
  );
}

function PricingSlide({ proposal, items }: { proposal: DeckProposal; items: DeckProposalItem[] }) {
  return (
    <SectionSlide label="Inversión" title="Detalle económico">
      <div className="max-w-3xl w-full">
        ...
        <PricingTotals proposal={proposal} />
      </div>
    </SectionSlide>
  );
}
...
function PricingBarChart({ proposal, items }: { proposal: DeckProposal; items: DeckProposalItem[] }) {
...
export function buildSlides(
  ...
  if (proposal.intro) {
    slides.push({ key: "intro", label: "Contexto", accent: "white", element: wm(<IntroSlide proposal={proposal} />) });
  }
  if (team.length > 0) {
