...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  if (error || !attachment) return NextResponse.json({ error: "Factura no encontrada" }, { status: 404 });
  ...
  const { data, error: downloadError } = await getStorage().download("documents", attachment.storage_path);
  if (downloadError || !data) return NextResponse.json({ error: "No se pudo leer el PDF" }, { status: 502 });
