...
import {
  Copy, Eye, EyeOff, Lock, LockOpen, Pencil, Plus, ShieldAlert, Trash2,
} from "lucide-react";
...
type VaultItem = {
  id: string; name: string; service: string; username: string | null;
  notes: string | null; is_sensitive: boolean; expires_at: string | null;
...
export function VaultClient({ items, passwordSet, unlocked, clients }: {
  items: VaultItem[]; passwordSet: boolean; unlocked: boolean; clients: Client[];
}) {
  ...
  function openEdit(item: VaultItem) { setEditItem(item); setDialog("edit"); }

  function handleReveal(item: VaultItem) {
    if (revealed[item.id]) { setRevealed((r) => { const n = { ...r }; delete n[item.id]; return n; }); return; }
    if (item.is_sensitive && !unlocked) { setDialog("unlock"); return; }
  ...
  function handleDelete(id: string) {
    ...
    startTransition(async () => { await deleteVaultItem({ id }); });
  }

  async function handleLock() {
    await lockVault(); window.location.reload();
  }

  const expiresLabel = (d: string | null) => {
    if (!d) return null;
    const diff = Math.ceil((new Date(d).getTime() - Date.now()) / 86_400_000);
    if (diff < 0) return <Badge variant="destructive" className="text-[10px] px-1">Caducado</Badge>;
    if (diff <= 30) return <Badge variant="warning" className="text-[10px] px-1">{diff}d</Badge>;
    return <Badge variant="neutral" className="text-[10px] px-1">{d}</Badge>;
  };
    ...
    <div className="flex flex-col gap-6">
      ...
      {passwordSet && !unlocked && (
        <div className="rounded-lg border border-amber-200 bg-amber-50 dark:border-amber-900/50 dark:bg-amber-950/30 px-4 py-3 text-sm text-amber-800 dark:text-amber-300 flex items-center gap-2">
          ...
          <span>Los secretos sensibles están ocultos. <button type="button" className="underline hover:no-underline" onClick={() => setDialog("unlock")}>Desbloquear bóveda</button></span>
        </div>
      )}
        ...
        <CardContent className="px-0 pt-0">
          {items.length === 0 ? (
            <Empty className="border-0 py-10">
              ...
              <EmptyContent>
                <Button size="sm" onClick={() => setDialog("add")}><Plus className="size-3.5" /> Añadir primera credencial</Button>
              </EmptyContent>
            </Empty>
          ) : (
                ...
                <thead>
                  <tr className="border-b border-border bg-muted/30 text-xs text-muted-foreground">
                    {["Nombre", "Servicio", "Usuario", "Secreto", "Caduca", ""].map((h, i) => (
                      <th key={i} className={cn("px-5 py-3 font-medium tracking-wide text-left", i === 5 && "w-20 text-right")}>{h}</th>
                    ))}
                  </tr>
                </thead>
                      ...
                      <tr key={item.id} className="group hover:bg-muted/40 transition-colors">
                        <td className="px-5 py-3 font-medium text-foreground">
                          <span className="flex items-center gap-1.5">
                            {item.name}
                            {item.is_sensitive && <Lock className="size-3 text-muted-foreground/60 shrink-0" />}
                          </span>
                        </td>
                        ...
                        <td className="px-5 py-3 text-muted-foreground font-mono text-xs">{item.username ?? <span className="opacity-30">—</span>}</td>
                        <td className="px-5 py-3">
                          <div className="flex items-center gap-2">
                            <span className={cn("font-mono text-xs", isRevealed ? "text-foreground select-all" : "text-muted-foreground tracking-widest")}>
                              {isRevealed ? revealed[item.id] : "••••••••"}
                            </span>
                            <button type="button" onClick={() => handleReveal(item)} className="text-muted-foreground hover:text-foreground transition-colors" aria-label={isRevealed ? "Ocultar" : "Revelar"}>
                              {isRevealed ? <EyeOff className="size-3.5" /> : <Eye className="size-3.5" />}
                            </button>
                            {isRevealed && (
                              <button type="button" onClick={() => navigator.clipboard.writeText(revealed[item.id]!)} className="text-muted-foreground hover:text-foreground transition-colors" aria-label="Copiar">
                        ...
                        <td className="px-5 py-3">
                          <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <Button variant="ghost" size="icon-xs" onClick={() => openEdit(item)} aria-label="Editar"><Pencil className="size-3.5" /></Button>
                            <Button variant="ghost" size="icon-xs" onClick={() => handleDelete(item.id)} aria-label="Eliminar" className="text-destructive hover:text-destructive hover:bg-destructive/10"><Trash2 className="size-3.5" /></Button>
                          </div>
                        </td>
                      </tr>
                    );
      ...
      <Dialog open={dialog === "setPassword"} onOpenChange={(o) => !o && setDialog(null)}>
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>
              {passwordSet ? "Verifica la contraseña actual antes de cambiarla." : "Define una contraseña para proteger los secretos sensibles. No se puede recuperar si la pierdes."}
            </DialogDescription>
          </DialogHeader>
