...
export function buildLeadJourneyAnalytics(
  ...
  const sources = new Map<
    string,
    { leads: number; won: number; pipelineValue: number; wonValue: number }
  >();
  let pipelineValue = 0;
  let wonValue = 0;

  for (const lead of leads) {
    const source = sourceName(lead.source, lead.utm_source);
    const leadValue = Number(lead.estimated_value ?? 0);
    const sourceStats = sources.get(source) ?? { leads: 0, won: 0, pipelineValue: 0, wonValue: 0 };
    sourceStats.leads += 1;
    sourceStats.pipelineValue += leadValue;
    pipelineValue += leadValue;
    sources.set(source, sourceStats);

    const journey = buildJourney(lead, eventsByLead.get(lead.id) ?? []);
    if (journey.includes("won")) {
      sourceStats.won += 1;
      sourceStats.wonValue += leadValue;
      wonValue += leadValue;
    }
...
export async function getLeadJourneyAnalytics(input: {
  ...
  const { data: leadData, error: leadError } = await notDeleted(
    supabase
      .from("leads")
      .select("id, source, utm_source, status, estimated_value, created_at")
