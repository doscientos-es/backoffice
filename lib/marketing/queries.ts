...
import {
  ...
  emptyAdCommercialPerformance,
  isMetaAttributedLead,
  type MarketingCustomerRow,
  type MarketingInvoiceRow,
  type MarketingLeadRow,
  mergeAdCommercialPerformance,
} from "./attribution";
...
/** Loads CRM outcomes for the same acquisition window as the Meta report. */
async function loadAdCommercialPerformance(
  adIds: string[],
  since: string,
  until: string,
): Promise<Map<string, AdCommercialPerformance>> {
  const empty = new Map(adIds.map((id) => [id, emptyAdCommercialPerformance()]));
  ...
  const { data: leadData, error: leadError } = await notDeleted(
    supabase
      .from("leads")
      .select("id, utm_content, status, estimated_value")
      .gte("created_at", since)
      .lte("created_at", `${until}T23:59:59.999Z`),
  );
  if (leadError) log.error({ err: leadError }, "marketing_crm_leads_query_failed");

  const leads = (leadData ?? []) as unknown as MarketingLeadRow[];
  const attributedLeads = leads.filter(
    (lead) => lead.utm_content && adIds.includes(lead.utm_content),
  );
  const leadIds = attributedLeads.map((lead) => lead.id);
  if (leadIds.length === 0) return empty;

  const { data: customerData, error: customerError } = await notDeleted(
    supabase.from("clients").select("id, lead_id").in("lead_id", leadIds),
  );
  if (customerError) log.error({ err: customerError }, "marketing_crm_customers_query_failed");

  const customers = (customerData ?? []) as unknown as MarketingCustomerRow[];
  const clientIds = customers.map((customer) => customer.id);
  let invoices: MarketingInvoiceRow[] = [];
  if (clientIds.length > 0) {
    ...
    if (invoiceError) log.error({ err: invoiceError }, "marketing_crm_invoices_query_failed");
    invoices = (invoiceData ?? []) as unknown as MarketingInvoiceRow[];
  }

  return buildAdCommercialPerformance(adIds, attributedLeads, customers, invoices);
}
...
export async function getActiveAdsOverview(opts: AdsOverviewOptions): Promise<MarketingOverview> {
  ...
  const crmPerformance = await loadAdCommercialPerformance(
    ads.map((ad) => ad.id),
    opts.since,
    opts.until,
  );
  ...
  const processedAds: ActiveAdRow[] = ads.map((ad) => {
    ...
    const crm = crmPerformance.get(ad.id) ?? emptyAdCommercialPerformance();
    ...
    return {
      ...
      qualifiedLeads: crm.qualifiedLeads,
      wonLeads: crm.wonLeads,
      lostLeads: crm.lostLeads,
      customers: crm.customers,
      wonValue: crm.wonValue,
      invoicedRevenue: crm.invoicedRevenue,
      ctr: agg.ctr,
      cpc: agg.cpc,
      cpl: agg.cpl,
...
export async function getCampaignsOverview(
  ...
  const byCampaign = new Map<
    string,
    { row: CampaignRow; insights: RawInsightRow[]; adIds: string[] }
  >();
  ...
  for (const ad of ads) {
    ...
    bucket.adIds.push(ad.id);
  }

  const campaigns: CampaignRow[] = Array.from(byCampaign.values()).map(
    ({ row, insights, adIds }) => {
      const agg = aggregateInsights(insights);
      const crm = adIds.reduce(
        (total, adId) =>
          mergeAdCommercialPerformance(
            total,
            crmPerformance.get(adId) ?? emptyAdCommercialPerformance(),
          ),
        emptyAdCommercialPerformance(),
      );
      return {
 ...
 *
 * Attribution chain: a Meta-attributed lead (instant form or paid-social UTM)
...
export async function getMarketingRoi(since: string, until: string): Promise<MarketingRoi> {
  ...
  // Commercial outcome for Meta leads only. Keeping this cohort aligned with
  // the Meta customer query below prevents organic/referral wins from inflating
  // the Meta conversion card.
  ...
  return computeMarketingRoi({
    spend,
    leads,
    totalLeads: metaPeriodLeads.length,
