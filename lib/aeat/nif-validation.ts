...
export type AeatFiscalIdentity = {
  nif: string;
  name: string;
};
...
/** Decodes only the documented VNifV2 response fields; never retains raw SOAP. */
    ...
    const request = https.request(
      ...
      (response) => {
        ...
        response.on("data", (chunk: Buffer) => {
          size += chunk.length;
          if (size > 1_048_576) request.destroy(new Error("La respuesta de AEAT excede el límite"));
